...
> ## STATE — generated 2026-08-17 17:22 UTC by `python scripts/checkpoint.py`
>
...
> | | |
> |---|---|
> | registered objects | **285** |
> | reaching a surface | **278 of 285** |
...
| **G29** | **Parcel→asset distance is measured wrong on the map** | 🔴 **OPEN — now MEASURED** | ⭐ **79,168 parcels have water physically ON them (0.0 mi)** and the map reports ~0.55 mi for every one. `in_water_distance_parcel` proves the correct method works. Apply the same `ST_DISTANCE` treatment to substations, lines and buses | `repPt()` uses the parcel's **first vertex** and one binned vertex per line, so a line crossing a parcel reads ~0.55 mi instead of **0.0**. Always overstates. ⭐ The screener already does this exactly in BigQuery — two methods in one app |
| **G30** | **🔴 GitHub Pages DEPLOY is failing** | 🔴 **OPEN — operator is getting failure emails** | Build SUCCEEDS, **deploy** fails. Cause is size, not code: `size-pack` **2.91 GiB**, working tree **369 MB**, of which **`data/sites/` alone is 334 MB** of committed parcel geometry. Pre-existing, not introduced this session. `.nojekyll` (`177a34e`) fixed a *separate* Jekyll-over-the-whole-repo failure. **Fix options: move `data/sites/` out of the deployed tree (PMTiles/CDN/release asset), or git-lfs, or a `docs/`-only Pages source.** ⚠ Removing it naively breaks the map — the console fetches `data/sites/{fips}.geojson.gz` on demand |
| **G31** | **NHD watershed backfill** | 🟡 **AGENT RUNNING at handoff** | Geometry covers **59 of 76** HUC8 (waterbodies 57). Agent dispatched with a DATA-COLLECTION-ONLY brief to close the ~18-watershed gap. Coverage gap, not correctness — every row present is valid |
