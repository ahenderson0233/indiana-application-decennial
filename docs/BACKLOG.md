...
> ## STATE — generated 2026-08-18 18:58 UTC by `python scripts/checkpoint.py`
>
...
> | | |
> |---|---|
> | registered objects | **299** |
> | reaching a surface | **282 of 299** |
...
| **G55** | **⚠ TARIFF ANALYSIS IS NOT FINISHED — 19 municipals loaded, NONE validated** | 🔴 **OPEN — the largest remaining tariff item, and it just became harder to hand-wave** | **Done:** the five IOUs are costed, audited, adapter-isolated and now fingerprinted (G54, G56). **252 municipal / co-op rows across 19 utilities are LOADED** — `in_utility_tariff_riders` at **668 rows / 73 utilities**, IOU rows byte-untouched at 334. ⛔ **Not one of those 252 rows has been through the costing audit** — and this session is the argument for why that matters: it found **nine further defects in the five utilities that had already been audited once**, including one that zeroed the rider stack on 17 of 18 schedules while every count on the page looked correct. The municipals are **19 publishers with 19 sets of conventions** against five, **none has an adapter**, so all fall back to the generic path, and **most have no EIA-861 industrial benchmark at all** — so the reconciliation gate that caught all seventeen IOU defects **does not exist for them**. A different check has to be designed first; `scripts/tariff_fingerprint.py` will at least say which of them move when it is. **Also outstanding:** the per-utility serviceability judgement in `docs/TARIFF_HARVEST_MUNIS_COOPS.md` reaches no surface — Richmond TS (~7.6¢/kWh at 90% LF) is the best published firm municipal path and the app never says so. ⚠ **Operational dependency: if the IURC loader re-runs, `load_tariff_books_munis_coops.py` MUST re-run after it** — the IURC loader's utility-scoped DELETE would remove these rows |
