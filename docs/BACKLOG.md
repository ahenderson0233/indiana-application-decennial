...
> ## STATE — generated 2026-08-18 15:13 UTC by `python scripts/checkpoint.py`
>
...
| # | task | state | notes |
|---|---|---|---|
| **⚠ IN FLIGHT** | **What is running or half-built RIGHT NOW — read before starting anything** | 🟡 **live 2026-08-18** | **(1) PJM case-23 WITHDRAWAL re-harvest**, `in_pjm_qs_c23sens_wd`, started 09:11, sole QueueScope process — injection is chained behind a poll-for-absence guard and a ≥1,500-bus landing check. ⛔ Never start a second QueueScope process; it was broken once today by a chained job whose `Wait-Process` failed open and launched injection 7 minutes in. Details and the two checkpoint-marker traps are in `docs/RESUME_HARVESTS.md`. **(2) A municipal/co-op tariff agent** is mid-run against sources outside IURC jurisdiction; it writes only to `in_utility_tariff_riders` scoped to its own utilities, so the five IOUs' 334 rows cannot be touched. **(3) G54**, the tariff costing, is actively being built — block parsing is proven (26/26 ladders parse) and the acceptance gate is reconciliation against EIA-861 realized averages. **(4) G51**'s three-state `row()` helper is shipped but its callers are not swept. |
| **G54** | **Cost each tariff to an annual spend and an effective ¢/kWh** | ✅ **DONE 2026-08-18 — and it RECONCILES** | Built on the CPS model’s method and **validated against what each utility’s industrial customers actually pay** (EIA-861): **I&M IP 8.00¢ vs 8.04¢ actual, AES HL 10.61¢ vs 10.48¢, SIGECO 8.58¢ vs 9.02¢, NIPSCO 631 5.78¢ vs 6.10¢, Duke HLF 10.46¢ vs 8.83¢.** ⭐ **The check is rendered per voltage on the page, not hidden** — a modelled bill that cannot be checked is a claim, not a calculation. **Five errors were found and fixed getting here, each recorded in the code:** (1) substring rider matching counted `HL1/HL2/HL3` as three riders on `HL`; (2) a class list reading *“HL (all voltages), PL”* was treated as a blanket and leaked HL riders onto SL and PH; (3) block ladders were SUMMED rather than allocated, putting NIPSCO at 57.94¢; (4) **`fuel_base` was added as a charge when it is already embedded in the energy charge** — the billable item is the FAC difference; (5) demand was billed flat rather than seasonally. ⭐ **A sixth was subtler and is the one worth remembering: choosing the CHEAPEST voltage as “best” selected whichever voltage had the LEAST DATA** — a lone customer charge reads 0.26¢/kWh, so a gap in our extraction masqueraded as a bargain. Runs now require an energy leg, anything under 2¢ is labelled **incomplete rather than cheap**, and voltages are ordered **physically (transmission first), never by price** — operator: *“the cheapest voltage may NOT be the one we interconnect to.”* Each voltage is modelled, judged and itemised separately. **Honest residuals:** I&M IP-LL is flagged a **modifier** (no energy leg of its own) rather than costed; NIPSCO 624/632/633 read 15–21¢ and that is the **inverted block ladder working as designed** to price high-load-factor customers out, which the page says rather than flagging as an error; reactive ($/kVAr) charges are excluded pending a power factor; and all-firm service is assumed, so a non-firm election moves the answer more than any rider | Operator: *"All of these rates should ideally be projected out to total a yearly spend and a per kWh cost based solely on the tariff (including the rate schedule and ALL applicable riders)."* **Two structural corrections from the operator are DONE and shipped:** a rider is not a schedule (Duke has **4 schedules and 14 riders**, not “18 schedules”), and riders now sit with the schedule they bill on rather than in a separate list. **The COSTING is what is withdrawn.** ⛔ **Three attempts, three wrong answers, each wrong for a different reason — recorded so the next attempt does not repeat them:** (1) **substring rider matching** — schedule `HL` matched rider rows for `HL1`/`HL2`/`HL3`, which are mutually exclusive VOLTAGE TIERS, so one rider counted three times and AES's stack read $28.89/kW-month; (2) **a class list read as a blanket** — `applies_to` of *“HL (all voltages), PL”* was treated as applying to every schedule, so HL riders also landed on SL and PH; (3) **BLOCK LADDERS SUMMED** — *“first 30,000 kWh”*, *“next 70,000 kWh”*, *“all over 1,000,000 kWh”* are ALTERNATIVES applied to successive slices, and summing them put NIPSCO at **57.94¢/kWh** against an Indiana industrial reality of roughly 6-9¢. ⭐ **Block detection now works and 6 of 18 schedules are flagged block-rated** (`BLOCK_RE`, compiled with import-time self-tests after the first version reached disk with literal BACKSPACE bytes where `\b` was intended and silently matched nothing). **Even so the survivors do not reconcile:** I&M IP-LL reads **1.77¢/kWh** (impossibly low) while Duke and AES read **~14-15¢** (too high); only NIPSCO 631 at 8.29¢ agrees with its own benchmark. ⛔ **ACCEPTANCE GATE for any future attempt — do not ship without it:** the modelled figure must reconcile against that utility's **EIA-861 realized industrial average**, which the Market page already holds, within a stated tolerance, per utility. **A bill that does not reconcile is not a bill.** **Still needed:** block ladders applied slice by slice; a power factor for the $/kVAr charges; a firm/non-firm election on tiered schedules (NIPSCO 631's Tier 2 is day-ahead LMP, which is not a fixed rate at all); and the demand ratchet, which sets billed demand in the months you are not running |
