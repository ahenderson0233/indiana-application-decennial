...
> ## STATE — generated 2026-08-17 23:19 UTC by `python scripts/checkpoint.py`
>
...
**READY-TO-RUN FIX for the one real case** (measured, not guessed):
- Source `energy.nonattainment_areas`; the Indiana slice is **clean — 35 distinct `(area_name,
  pollutant_name)` keys, 0 duplicated, 0 with conflicting classifications**.
- Our clip `in_nonattainment` holds **62 rows over those 35 keys** — so the clip carries duplicate
  keys, which is why a naive `UPDATE … FROM parent` fails with *"must match at most one source row"*.
- **Fix:** write a NEW table `in_nonattainment_classified` (do not mutate the working clip), joining
  on `UPPER(TRIM(area_name))` + `UPPER(TRIM(pollutant_name))` with the **source GROUP BY-deduped**
  (safe *because* conflict count is 0 — measured). Add `classification`,
  `classification_effective_date`, `classification_citation`, `classification_url`. Register in the
  same run. Vocabulary present: Former Subpart 1 (14), Marginal (5), Moderate (4), Serious (2), …
