...
> ## STATE — generated 2026-08-18 16:11 UTC by `python scripts/checkpoint.py`
>
...
> | | |
> |---|---|
> | registered objects | **298** |
> | reaching a surface | **282 of 298** |
...
| # | task | state | notes |
|---|---|---|---|
| **G55** | **⚠ TARIFF ANALYSIS IS NOT FINISHED — the municipals are loaded but NOT validated** | 🔴 **OPEN — the largest remaining tariff item** | **What IS done:** the five IOUs are costed and audited (G54), and **252 municipal / co-op rows across 19 utilities landed** — `in_utility_tariff_riders` is at **668 rows / 73 utilities**, zero not_held-with-a-rate violations, IOU rows byte-untouched at 334. ⛔ **What is NOT done: none of those 252 municipal rows has been through the costing audit.** Every defect in G54 was found by running the calculator over the IOUs and comparing against EIA-861; **the municipals have had no equivalent pass**, and they are MORE likely to break it, not less — they are 19 separate publishers with 19 sets of conventions, versus five. **Most have no EIA-861 industrial benchmark at all**, so the reconciliation gate that caught the IOU errors simply does not exist for them; a different check is needed. **Also outstanding:** the per-utility serviceability judgement is written in `docs/TARIFF_HARVEST_MUNIS_COOPS.md` but reaches no surface; Richmond TS (~7.6¢/kWh at 90% LF) is the best published firm municipal path and the app does not say so. ⚠ **Operational dependency, carried in both registries: if the IURC loader is re-run, `load_tariff_books_munis_coops.py` MUST be re-run after it** — the IURC loader's utility-scoped DELETE would remove these rows |
| **G56** | **Rewrite the tariff parser as PER-UTILITY adapters, not one generic matcher** | 🔴 **OPEN — operator, 2026-08-18** | Operator: *"It may be best to just write code for the individual utilities rather than trying to aggregate them all into one code block."* ⭐ **The evidence is overwhelming and it is all in G54: nine defects, five utilities, and every single one was a generic rule meeting a house convention** — AES writes *low-load-factor transmission*, Duke writes *primary direct* and splits *transmission* into two kV bands, NIPSCO joins classes with a slash, I&M punctuates *I.P.*, and one Duke row states a floor in a basis containing the word *maximum*. Several fixes for one utility broke another: separating AES's LLF class fragmented its demand leg; making multi-class components class-agnostic put Duke HLF at +402%. **A shared arithmetic core with a small declarative adapter per utility** — its schedule codes, its rider→schedule mapping, its service-class vocabulary — is more code but each piece is independently verifiable, and a change for one utility CANNOT break another. ⚠ Do this **before** attempting the 19 municipals (G55): 19 more publishers through one generic matcher is the same trap, four times over |
| **⚠ IN FLIGHT** | **What is running or half-built RIGHT NOW — read before starting anything** | 🟡 **live 2026-08-18** | **(1) PJM case-23 WITHDRAWAL re-harvest**, `in_pjm_qs_c23sens_wd`, started 09:11, sole QueueScope process; injection is chained behind a poll-for-absence guard and a ≥1,500-bus landing check. ⛔ Never start a second QueueScope process — that rule was broken once today by a chained job whose `Wait-Process` failed open. See `docs/RESUME_HARVESTS.md` for the two checkpoint-marker traps. **(2) Tariff costing (G54)** is working for the five IOUs and **not validated for the 19 municipals (G55)**. **(3) G56** proposes rewriting the parser per-utility before the municipals are attempted. **(4) G51**'s three-state `row()` helper is shipped, callers not swept. **(5) G48** has measured groundwork, not built. |
| **G54** | **Cost each tariff to an annual spend and an effective ¢/kWh** | 🟡 **PARTLY DONE — NOT FINISHED** | **Working and validated for the schedules that matter most**, checked against each utility's EIA-861 **industrial** realized average (operator's call: industrial, not C&I, because we are a large-load customer). **Current audit, 300 MW at 85% LF: 26 priced service-class rows, 15 within ±20%, 11 outside, 2 refused as not costable, 4 schedules excluded on a real eligibility ceiling.** ⭐ **I&M IP-LL** — the large-load schedule — is the best behaved: 7.06 / 7.45 / 8.56 / 9.54¢ across four service classes, all within ±20%, with all 8 riders and the embedded capacity charge itemised. **NINE distinct defects were found and fixed, every one of them a generic parser meeting a utility-specific convention:** (1) substring rider matching counted `HL1/HL2/HL3` as three riders on `HL`; (2) *“HL (all voltages), PL”* read as a blanket, leaking riders onto SL and PH; (3) block ladders SUMMED — NIPSCO at 57.94¢; (4) `fuel_base` added as a charge when it is already embedded in the energy rate; (5) demand billed flat rather than seasonally; (6) the cheapest voltage chosen as “best”, which selected whichever class had the LEAST DATA (a lone customer charge reads 0.26¢); (7) AES's *low-load-factor transmission* summed with plain *transmission* — 20.64¢, dearer than secondary, which was the tell; (8) NIPSCO's `transmission/subtransmission` slash unrecognised, so a $35.74/kW-month demand charge bound to sub-transmission alone and the transmission row showed **DEMAND $0**; (9) I&M writes *“Tariff I.P.”* with periods, so tokenising gave `I` and `P` and never `IP` — **all eight I&M riders, about +8.6 $/kW-month, silently failed to attach.** ⭐ **The guard that matters more than any single fix: a row missing a whole billing leg now refuses to show an effective rate.** Every one of those bugs shared that signature, and it is the only reason they were findable. ⚠ **STILL OPEN:** NIPSCO 631's service classes degrade to *“aggregated premises”* / *“all tiers”* rather than real voltages, and its +52/+69% is not yet explained; AES PH and I&M GS are correctly refused but not repaired; Duke HLF has not been re-verified since the multi-class fix because its ceiling excludes it; and a flat ±20% band is the wrong instrument — a 300 MW customer SHOULD sit below an average that includes small industrials, so the tolerance must scale with load |
