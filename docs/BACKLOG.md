...
> ## STATE — generated 2026-08-17 21:22 UTC by `python scripts/checkpoint.py`
>
...
| **G27** | **401 columns under-clipped across 24 tables** | 🟡 **PARTLY DONE** | ✅ `percent_loading_before` + `headroom_state` re-clipped — the app can now say WHY a bus reads zero (641 of 642 zeros are pre-existing overloads). **Remaining:** gas asset tables have no coordinates, `echo_cwa_facilities` has `faclong` and no `faclat`, `in_nonattainment` lost `classification` | Incl. **`percent_loading_before`** — the exact column needed to tell "already overloaded" from "genuinely full". Gas assets lost their coordinates; `in_nonattainment` lost `classification`. Detail in `docs/CLIP_COMPLETENESS.json` |
