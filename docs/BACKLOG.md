...
> ## STATE — generated 2026-08-18 17:04 UTC by `python scripts/checkpoint.py`
>
...
| # | task | state | notes |
|---|---|---|---|
| **G55** | **⚠ TARIFF ANALYSIS IS NOT FINISHED — 19 municipals loaded, NONE validated** | 🔴 **OPEN — the largest remaining tariff item** | **Done:** the five IOUs are costed, audited and now have per-utility adapters (G54, G56); **252 municipal / co-op rows across 19 utilities are LOADED** — `in_utility_tariff_riders` at **668 rows / 73 utilities**, zero not_held-with-a-rate violations, IOU rows byte-untouched at 334. ⛔ **NOT done, and this is the point: not one of those 252 rows has been through the costing audit.** Every defect in G54 was found by running the calculator over the IOUs and reconciling against EIA-861. The municipals have had **no equivalent pass**, they are **19 separate publishers with 19 sets of conventions** against five, **none has an adapter** so all fall back to the generic path that produced nine defects on the IOUs, and **most have no EIA-861 industrial benchmark at all** — so the reconciliation gate that caught those errors does not exist for them and a different check must be designed. **Also outstanding:** the per-utility serviceability judgement sits in `docs/TARIFF_HARVEST_MUNIS_COOPS.md` and reaches no surface — Richmond TS (~7.6¢/kWh at 90% LF) is the best published firm municipal path and the app never says so. ⚠ **Operational dependency, carried in both registries: if the IURC loader re-runs, `load_tariff_books_munis_coops.py` MUST re-run after it** — the IURC loader's utility-scoped DELETE would remove these rows |
| **G56** | **Rewrite the tariff parser as PER-UTILITY adapters, not one generic matcher** | ✅ **DONE 2026-08-18** — `scripts/tariff_adapters.py`, one declarative adapter per IOU, shared arithmetic left in `export_tariffs.py`. Each adapter may declare `applies_to_norm`, `class_separators`, `split_classes` / `merge_classes`, `explicit_classes`, `parent`, `large_load` / `not_large_load`, `bounds_from_name_only` and a prose `notes` — and the notes now ship in the payload, so the publisher's conventions reach the page rather than living in a commit message. **Verified as a refactor, not a rewrite of behaviour: 15 of 18 IOU schedules are structurally IDENTICAL before and after**, and the three that moved all GAINED service classes the generic matcher had collapsed (Duke HLF 1→2, AES PH 0→1, SIGECO LP 1→2). ⭐ It immediately earned its keep: NIPSCO 631's classes had degraded to *“all tiers”* and *“aggregated premises with BTM generation”* because its slash-joined demand row is (correctly) class-agnostic, leaving nothing to infer from. One line of `explicit_classes` in the NIPSCO adapter states what its eligibility says outright — transmission / sub-transmission — and it is declared in NIPSCO's own section where it cannot affect anyone else. ⚠ **The 19 municipals have NO adapter and fall back to the generic path (G55).** |
...
| **G54** | **Cost each tariff to an annual spend and an effective ¢/kWh** | 🟡 **WORKING FOR THE 5 IOUs — NOT FINISHED** | Validated against each utility's EIA-861 **industrial** realized average (operator's call — industrial, not C&I, because the customer is large load). **Audit at 300 MW / 85% LF: 27 priced service-class rows, 15 within ±20%, 12 outside, 2 refused as not costable, 4 schedules excluded on a real eligibility ceiling.** ⭐ **I&M IP-LL — the large-load schedule — is the best behaved: 7.06 / 7.45 / 8.56 / 9.54¢ across four service classes, all within ±20%, with all eight riders and the embedded capacity charge itemised.** **NINE defects found and fixed, every one a generic rule meeting a house convention:** substring rider matching (`HL` caught `HL1/HL2/HL3`); a class list read as a blanket; block ladders SUMMED (NIPSCO at 57.94¢); `fuel_base` added when it is already embedded in the energy rate; demand billed flat not seasonally; “cheapest voltage” selecting whichever class had the LEAST DATA (0.26¢); AES's low-load-factor transmission summed with plain transmission (20.64¢, dearer than secondary — the tell); NIPSCO's `transmission/subtransmission` **slash** leaving the transmission row with **DEMAND $0** on a $35.74/kW-month schedule; and I&M's **“Tariff I.P.”** periods, which stopped **all eight** of its riders (~+8.6 $/kW-month) from attaching. **Those conventions now live in per-utility adapters (G56).** ⭐ **The guard that matters more than any single fix: a row missing a whole billing leg refuses to show an effective rate.** Every one of the nine shared that signature. ⚠ **STILL OPEN:** NIPSCO 631 reads +52/+69% and is not explained (its classes are now correct, so the cause is elsewhere — likely the Tier 2/3 market-indexed legs being treated as firm); AES **PH** and I&M **GS** are correctly refused but not repaired; Duke **HLF** has never been re-verified since the multi-class fix because its ceiling excludes it; and **a flat ±20% band is the wrong instrument** — a 300 MW load SHOULD sit below an average that includes small industrials, so tolerance must scale with load |
