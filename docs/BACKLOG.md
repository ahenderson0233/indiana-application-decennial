...
> ## STATE — generated 2026-08-18 01:27 UTC by `python scripts/checkpoint.py`
>
...
| **G37** | **PJM case-23 harvests (the benchmark's exact PJM study)** | 🟡 **RESUMED after a network drop — see `docs/RESUME_HARVESTS.md`** | Case 23 = *"2028 TC2 Phase II Case (Summer Peak)"* = the vendor's exact PJM case, **1,826 AEP buses vs 1,524 in case 4**. Both directions harvesting; a connection drop killed them mid-run and they resumed cleanly from on-disk checkpoints. ⛔ **Owner id is `1568` for case 23, NOT 739** — QueueScope renumbers per case and 739 loads 0 rows while exiting *successfully*. ⛔ Never two QueueScope instances; ⛔ never delete `data/` (checkpoint markers are gitignored — deletion forces a duplicating re-harvest). **On completion: rebuild `in_bus_capacity_tier0` on case 23, attach the G7m cost join, retire the case-4 tables (G23)** |
